...
install_launcher() {
    ...
    # Use a plain `cd` then `exec` — `env -C` is a GNU extension and
    # breaks on macOS. This form runs on both with no surprises.
    #
    # The `update` subcommand is intercepted here rather than inside the
    # Python entry point because updating git-tracked code under a
    # running Python process would require a restart anyway. Doing it in
    # the launcher keeps the Python side free of installer concerns.
    cat > "$launcher" <<EOF
#!/usr/bin/env bash
...
if [ "\${1:-}" = "update" ]; then
    if [ ! -d "\$INSTALL_DIR/.git" ]; then
        echo "pod-the-trader: no git checkout at \$INSTALL_DIR — re-run the installer." >&2
        exit 1
    fi
    echo "Updating pod-the-trader from \$REPO_URL (\$REF)..."
    cd "\$INSTALL_DIR"
    ...
    uv sync --quiet
    echo "Done. Run 'pod-the-trader' to launch."
    exit 0
fi
